...
func InitDb(addr string) *ethpebble.Database {
	pebbleDB, createErr = ethpebble.New(addr, 400, 65536, "eth/db/chaindata/", false, false)
	if createErr != nil {
...
func (job *Job) WriteBatchToPebble() error {
	if len(job.Kvbuffer) > 0 {
		...
		if err := kvBatch.Write(); err != nil {
			log.Error("send kv rocks error", "err", err.Error())
			panic("write err")
			return err
		}
	...
	go func() {
		for {
			...
			case job := <-w.JobChannel:
				...
				if err := job.WriteBatchToPebble(); err != nil {
